  ...
  <title> </title>
	...
	function calcular(select) {
		...
		if(select.options[select.selectedIndex].id == "boleta"){
			...
			var totalfinalFormat2 = parseFloat(totalfinal).formatMoney(0,"",".",".");
			
			
			document.getElementById("totalfinalcampo").value = totalfinalFormat2;
  ...
  <div id="firmas">
    <div class="grupo">
      <?php
		...
		$jefe_a = $_REQUEST['jefe_autorizacion'];
		...
		if($username_from_sesion=='laura.espinozamonroy@cencosud.cl'){
			...
			if($jefe_a=='Andrea Montes'){
				echo "<div id=\"firma-1\"><img src=\"tema/img/andrea-montes.jpg\"></div>";
			}
			if($jefe_a=='Roberto Moore'){
				echo "<div id=\"firma-2\"><img src=\"tema/img/firma-2.jpg\"></div>";
			}
			if($jefe_a=='Rose Marie Villalón'){
				echo "<div id=\"firma-1\"><img src=\"tema/img/rose.jpg\"></div>";
			}
			if($jefe_a=='Macarena Borquez'){
				echo "<div id=\"firma-1\"><img src=\"tema/img/macarena-borquez.jpg\"></div>";
			}
	...
	<div class="grupo">
		  ...
		  <div class="caja base-70 no-padding">
		  	<div class="PEP">
			<?php
				//$last_id trae el ultimo id de la ultima OC que se inserto
				
				$registrosLastOrden = mysqli_query($conexion, "SELECT * FROM ordenes WHERE numero_orden = '$last_id' ") or die("Problemas en el select de ultima orden".mysqli_error($conexion));
