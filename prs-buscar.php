...
<?php    	
	...
	$buscar = @$_REQUEST["palabra"];
		...
		if($buscar!=''){
			...
			$consulta_mysql=mysqli_query($conexion,"SELECT * FROM ordenes WHERE numero_orden = '$buscar' AND archivo = 'no'") or die("Problemas en el select:".mysqli_error($conexion));
		...
		if($buscar_rut!=''){
			...
			if($num_registros_pro!=0){
				...
				//echo $buscar;
				//Ojo esto es para buscar una orden en especifica
				$consulta_mysql=mysqli_query($conexion,"SELECT * FROM ordenes WHERE id_proveedor = '$id_proveedor' AND archivo = 'no'") or				
				die("Problemas en el select:".mysqli_error($conexion));
	...
	<br>
	 <a href="por-revisar-sap.php"><input type="button" value="Volver"></a>	
