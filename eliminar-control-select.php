...
<html lang="es">
  ...
  <body>
	...
	$registrosControl=mysqli_query($conexion,"select * from control_presupuesto") or
    ...
    <div id="data--input" class="grupo">
      <h3>Administrador</h3>
	  <h4>Eliminar Control Presupuesto</h4>
	  <h6><a href="gestionar-control.php">Volver</a></h6>
	  <h6>Seleccione ID del Control Presupuesto que desea eliminar.</h6>
    </div>
    <section class="grupo">
      <div class="nav-admin">
        <ul>
      ...
      <table class="table-sap" style="width: 40% !important;">
        ...
        <tbody>
			<?php
				...
				while($reg=mysqli_fetch_array($registrosControl)){
					$id_controlP = $reg['id_controlP'];
					$id = $reg['id'];
					$control_presupuesto = $reg['control_presupuesto'];
				
					echo "<tr>";
						echo "<td class=\"area\"><a href=\"eliminar-control.php?id_control_s=",urlencode($id_controlP)," \">$id_controlP</a></td>";
						echo "<td class=\"ceco\">$id</td>";
						echo "<td class=\"desc-servicio\">$control_presupuesto</td>";						
					echo "</tr>";
				}
