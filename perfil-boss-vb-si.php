  ...
  <body>
    ...
    <div id="buscar" class="grupo">
	  ...
	  <!-- Mod Mod Mod Mod Mod Mod Mod Mod Mod Mod Mod Mod Mod Mod -->
        <form id="" method="POST" action="pbvbsi-buscar.php" onSubmit="return validarForm(this)" class="seek"> 
          <input type="search" name="palabra" placeholder="ingresa número de OC">
          <button type="submit" value="Buscar" name="buscar">buscar</button>
        </form>
		<form id="" method="POST" action="pbvbsi-buscar.php" class="seek"> 
	...
	if(isset($_POST['buscar'])){   
		...
		if($buscar!=''){
			...
			$consulta_mysql = mysqli_query($conexion,"SELECT * FROM ordenes WHERE numero_orden = '$buscar' AND jefe_autorizacion = '$nombre_final' AND visto_bueno = \"si\" ") or die("Problemas en el select:".mysqli_error($conexion));
	...
	if(isset($_POST['rut_proveedor'])){   
		...
		if($buscar_pro!=''){
			...
			if($num_registros_pro!=0){
				...
				//Ojo esto es para buscar una orden en especifica
				$consulta_mysql = mysqli_query($conexion,"SELECT * FROM ordenes WHERE id_proveedor = '$id_proveedor' AND jefe_autorizacion = '$nombre_final' AND visto_bueno = \"si\" ") or die("Problemas en el select:".mysqli_error($conexion));
